/* browser-roguelike#f129cf6 - James Pound @ 2021-05-26 16:45 */
"use strict";
...
function generateRandomLevel(currentLevel, height, width) {
  ...
  function createRoom(id) {
    ...
    rooms.push(new Room(id, 30, 0)); // If there's an overlap with another room, delete the new room and try creating a new one

    if (overlapCheck(id)) {
      rooms.pop();
      tries += 1; // Don't try more than 5 times
  ...
  function overlapCheck(id) {
    var roomChecking = rooms[id]; // Compare the room to the other rooms

    for (var comparingIndex = 0; comparingIndex < rooms.length; comparingIndex++) {
      ...
      if (roomChecking.id != roomComparingTo.id) {
        ...
        // Check the top left
        if (between(roomComparingTo.corners.topLeft[0], roomChecking.corners.topLeft[0], roomChecking.corners.bottomRight[0]) && between(roomComparingTo.corners.topLeft[1], roomChecking.corners.topLeft[1], roomChecking.corners.bottomRight[1])) {
          console.log('Top left of room ' + comparingIndex + ' is in range of room ' + id);
          return true;
        } // Check the top right


        if (between(roomComparingTo.corners.topRight[0], roomChecking.corners.topLeft[0], roomChecking.corners.bottomRight[0]) && between(roomComparingTo.corners.topRight[1], roomChecking.corners.topLeft[1], roomChecking.corners.bottomRight[1])) {
          console.log('Top right of room ' + comparingIndex + ' is in range of room ' + id);
          return true;
        } // Check the bottom left


        if (between(roomComparingTo.corners.bottomLeft[0], roomChecking.corners.topLeft[0], roomChecking.corners.bottomRight[0]) && between(roomComparingTo.corners.bottomLeft[1], roomChecking.corners.topLeft[1], roomChecking.corners.bottomRight[1])) {
          console.log('Top right of room ' + comparingIndex + ' is in range of room ' + id);
          return true;
        } // Check the bottom right


        if (between(roomComparingTo.corners.bottomRight[0], roomChecking.corners.topLeft[0], roomChecking.corners.bottomRight[0]) && between(roomComparingTo.corners.bottomRight[1], roomChecking.corners.topLeft[1], roomChecking.corners.bottomRight[1])) {
          console.log('Top right of room ' + comparingIndex + ' is in range of room ' + id);
          return true;
