/* gh-pages#222d3c3 - James Pound @ 2021-08-02 11:54 */
"use strict";
...
function Enemy(elem, id, pos, type, health) {
  ...
  this.type = type;
  this.health = health;
  this.pos = pos; // Style element for rendering each enemy's position
...
var MainGameCode = /*#__PURE__*/function () {
  function MainGameCode() {
    var _this = this;

    _classCallCheck(this, MainGameCode);

    document.querySelector('head').appendChild(overrides);
    document.querySelector('head').appendChild(zoomLevelStyle);
    document.querySelector('head').appendChild(stylePlayer); // Start touch controls

    document.addEventListener('touchstart', this.handleTouchStart, false);
    document.addEventListener('touchmove', this.handleTouchMove, false);
    window.addEventListener('resize', this.centerPlayerInScreen); // Handle input
  ...
  _createClass(MainGameCode, [{
    key: "drawScreen",
    value: function drawScreen(selectedLevel) {
      var _this2 = this;
      ...
      switchCameraBtn.addEventListener('click', function () {
        _this2.toggleCenterMode();
      });
      showGoalBtn.addEventListener('click', function () {
        _this2.showGoal();
      });
      zoomUp.addEventListener('click', function () {
        ...
        _this2.renderPlayer(player.pos);

        _this2.renderEnemies();

        _this2.centerPlayerInScreen();

        grid.classList.remove('no-anim');
      });
      zoomDown.addEventListener('click', function () {
    ...
    key: "drawTitleScreen",
    value: function drawTitleScreen() {
      var _this3 = this;
      ...
      uiElem.appendChild(messageWindow);
      uiElem.appendChild(titleContainer); // Show title screen
      ...
      btnStartNormal.addEventListener('click', function () {
        closeTitlescreen();

        _this3.drawScreen(levelData[0]);
      });
      btnStartProcudural.addEventListener('click', function () {
        _this3.displayMessageBox('This mode is in development and is not yet available.', 'Dismiss', 'dismiss');
      });
    }
  }, {
    key: "drawDecorations",
    value: function drawDecorations() {
    ...
    key: "centerPlayerInScreen",
    value: function centerPlayerInScreen() {
      ...
      var topLevelOffset = (levelStore[currentLevel].length / 2 - player.pos[0]) * sessionStats.zoomLevel * 4;
      var leftLevelOffset = (levelStore[currentLevel][0].length / 2 - player.pos[1]) * sessionStats.zoomLevel * 4;
      var top;
      var left;
    ...
    key: "moveEnemy",
    value: function moveEnemy(enemyObject, direction) {
      ...
      if (levelStore[currentLevel][newPos[0]][newPos[1]].type != 'wall' && levelStore[currentLevel][newPos[0]][newPos[1]].inside.indexOf('enemy') === -1) {
        ...
        this.renderEnemy(enemyObject, newPos); // Update enemy object
    ...
    value: function movePlayer(direction) {
      ...
      if (levelStore[currentLevel][newPos[0]][newPos[1]].type != 'wall') {
        // Update the visuals
        this.renderPlayer(newPos); // Update the levelStore
    ...
    key: "renderPlayer",
    value: function renderPlayer(pos) {
    ...
    key: "renderEnemy",
    value: function renderEnemy(enemyObj, pos) {
    ...
    key: "cleanupEnemyStyles",
    value: function cleanupEnemyStyles(level) {
    ...
    key: "goToNewLevel",
    value: function goToNewLevel(newLevel) {
      sessionStats.turnsLevel = 0;
      enemyCounter = 0; // Erase screen

      this.eraseScreen(); // Clean up enemy stlye elements in head
    ...
    key: "displayMessageBox",
    value: function displayMessageBox(messageText, btnText, action) {
    ...
    key: "submitHighscore",
    value: function submitHighscore() {
    ...
    key: "displayVictoryMessage",
    value: function displayVictoryMessage() {
      var messageBox = document.querySelector('#message');
      var message = document.createElement('p');
      var button = document.createElement('a');
