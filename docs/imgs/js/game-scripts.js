...
function showGoal() {
  ...
  if (viewingGoal) {
    ...
    top =
      goal.offsetTop * -1 - sessionStats.zoomLevel * 4 + window.innerHeight / 2;
    left =
      goal.offsetLeft * -1 - sessionStats.zoomLevel * 4 + window.innerWidth / 2;
...
function moveEnemy(enemyObject, direction) {
  ...
  if (
    ...
    if (
      levelStore[currentLevel][newPos[0]][newPos[1]].inside.indexOf('player') >
      -1
    ) {
      death();
    }
...
function movePlayer(direction) {
  ...
  // Ran into an enemy
  if (
    levelStore[currentLevel][newPos[0]][newPos[1]].inside.indexOf('enemy') > -1
  ) {
    death();
    return;
  }
